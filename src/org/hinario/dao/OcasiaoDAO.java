...
public class OcasiaoDAO extends DAOBase implements Serializable {
	...
	public List<? extends EntidadeBase> getLista(Integer inicio, Integer limite, List<SortMeta> multiSortMeta, Filtro filtro) {
		TypedQuery<Ocasiao> q = (TypedQuery<Ocasiao>) getQueryOrdenadaEFiltrada("select ocasiao from Ocasiao ocasiao", "ocasiao", em, filtro, multiSortMeta, Ocasiao.class);
		if (inicio != null)
			q.setFirstResult(inicio);
		if (limite != null)
			q.setMaxResults(limite);
		List<Ocasiao> returN = q.getResultList();
		return returN;
	}
